...
function MealSection({
  ...
  const subtotal = items.reduce((sum, e) => sum + (e.calories ?? 0), 0);
  const protein = items.reduce((sum, e) => sum + (e.protein ?? 0), 0);
  const carbs = items.reduce((sum, e) => sum + (e.carbs ?? 0), 0);
  const fat = items.reduce((sum, e) => sum + (e.fat ?? 0), 0);
